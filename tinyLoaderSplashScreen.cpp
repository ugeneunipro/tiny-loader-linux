...
void drawSplashScreen(int *isDownloaded) {
    char *filename = (char *)"/home/ichebyki/CLionProjects/ugeneloader/toucan.pbm";
    unsigned int verbose = 0;
    Image *newimage = loadImage(filename, verbose);
    ...
    if (newimage) {
        imageInWindow(display, screen, newimage,
                      0, NULL, verbose);
    }
    ...
    XChangeProperty(display, frame_window, type, XA_ATOM, 32, PropModeReplace,
    ...
    for (;;) {
        ...
        if (i == 0) {
            XClearArea(display, frame_window, text_x, text_y - (text_structure.ascent + text_structure.descent),
                       text_structure.width, text_structure.ascent + text_structure.descent, False);
            XDrawString(display, frame_window, graphical_context,
                        text_x, text_y, hello_string1, hello_string_length);
            XFlush(display);
            usleep(500000);
